...
- `{{branch_name}}`: deterministic work branch. Defaults to `research/{{project_slug}}/{{topic_slug}}`.
...
- `{{final_branch_commit}}`, `{{main_merge_commit}}`, `{{outline_rounds}}`, `{{deep_rounds}}`: TW handoff inputs (composable mode only).
...
- Never write research artifacts on a random runtime branch; switch to `branch_name` before persisting.
- Never post Research Complete unless Orchestrator has provided `main_merge_commit`. In single-issue lightweight mode (no `report_issue_id`), Research Complete and TW handoff are skipped — pipeline ends at Final Promotion Ready, and Orchestrator integrates to main and posts the closing comment directly.
